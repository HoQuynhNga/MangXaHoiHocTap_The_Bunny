...
// =========================================================================
// 4. THUẬT TOÁN ĐỌC VÀ THỰC THI FILE SQL (XỬ LÝ ĐƯỢC CẢ DELIMITER)
// =========================================================================

// Đường dẫn tới file .sql của bạn (Hãy sửa lại tên file cho đúng với file của nhóm)
$sql_file_path = 'database/seed-demo-full.sql'; 
...
foreach ($lines as $line) {
    ...
    if (substr($trimmed_line, -strlen($delimiter)) === $delimiter) {
        ...
        // Xóa delimiter ở cuối chuỗi để tránh lỗi cú pháp MySQLi
        $query_to_execute = substr(trim($query), 0, -strlen($delimiter));

        if ($conn->query($query_to_execute) === TRUE) {
            $success_count++;
        } else {
            // In ra dòng SQL bị lỗi để dễ fix bug
            echo "<span class='text-danger'>❌ Lỗi: " . $conn->error . "</span><br>";
            echo "<span class='text-secondary'>Query: " . htmlspecialchars(substr($query_to_execute, 0, 100)) . "...</span><br><br>";
            $error_count++;
        }
...
if ($error_count === 0 && $success_count > 0) {
    ...
    echo "<div class='alert alert-warning'><h5>⚠️ Import hoàn tất nhưng có lỗi</h5>Đã thực thi $success_count lệnh thành công, nhưng có <strong>$error_count</strong> lệnh bị lỗi. (Xem chi tiết trên khung đen).</div>";
} else {
